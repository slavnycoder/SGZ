...
def main(args):
    ...
    with open(test_sample_list_file) as f:
        ...
        for line in reader:
            ...
            # compare the results
            expected_files = [os.path.join(expectedfolder, '%s.basic.sgz.txt' % sample),
                              os.path.join(expectedfolder, '%s.fmi.sgz.txt' % sample),
                              os.path.join(expectedfolder, '%s.fmi.sgz.full.txt' % sample)]
            result_files = [os.path.join(outputfolder, '%s.basic.sgz.txt' % sample), os.path.join(outputfolder, '%s.fmi.sgz.txt' % sample),
                            os.path.join(outputfolder, '%s.fmi.sgz.full.txt' % sample)]
...
def _arg_parser():
    ...
    g.add_argument('-s', '--test_sample_list_file', action='store', type=str,
                   default=os.path.join(os.getcwd(), 'test', 'test_sample_list.txt'),
